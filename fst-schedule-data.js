...
//   「EVENT SCHEDULE 2026.09.19-23」を2026-09-01にPDFの書き起こしデータとして取得し、
...
//              別途確認済みの既知の複合表記をそのまま文字列で当てた行(下記MAIN_DAY1参照)。
//              すでに「¥50,000 ／ …」のような完成した表記のため、円マーク等を二重に付けずそのまま表示する。
//   - null   … PDF・別途確認のいずれからも金額を特定できていない行。金額を推測で埋めず、
...
//      2026-09-01の確認により、既存の const FST.events[0].entry(index.html。MAIN EVENT概要欄)と
...
//      PDF上のENTRY欄が数値ではなくアイコン/バッジ表記だったため当初 null にしていたが、
//      2026-09-01の確認により、既存の const FST.events[1].entry(index.html。CHAMPIONSHIP概要欄)と
//      同一の「¥30,000 ／ FSTチケット2枚 ／ ¥20,000＋FSTチケット1枚」であることが判明したため、
//      この文字列を明示的に設定している(推測ではなく確認済みの値)。
//      【2026-09-09追記・訂正】上記「¥20,000」は誤りだった。一次情報の追加調査
...
//      ¥15,000＋FSTチケット1枚」と判明。同サイトの /schedule ページを直接確認し、
