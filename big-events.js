 ...
 *
 * ■ 掲載期間ルール(2026-07-29に14日で設定 / 2026-09-01に30日へ変更 / 2026-09-06に上限を時刻化)
 ...
 *   ⇒ 掲載期間は **重なってよい**。トップのバナーは同時に複数件出る(仕様として了承済み)。
...
const BIG_EVENTS = [
  {
    ...
    bannerClass: 'ev-wjpt',
    // ---- WJPT 2026 サテライト開催実績(2026-08-28集計。終了済み大会のアーカイブ) ----
    ...
    // 目視での参考情報はKENポーカー久留米(v21)・KKPOKER FUKUOKA(v2)の2店舗のみだったが、
  ...
  },
  {
    ...
    bannerClass: 'ev-jopt',
    // ---- JOPT 2026 福岡 #01 サテライト開催実績(2026-08-28集計。終了済み大会のアーカイブ) ----
    // 判定基準・再集計方法はWJPTと同じ(正規表現を JOPT に読み替える)。
    // 目視での参考情報はKKPOKER FUKUOKA(v2)・RAFTEL CASINO(v37)・Poker room SKY(v33)の
  ...
  },
  {
    ...
    // ---- 日本シリーズ 2026 福岡 サテライト開催実績(2026-08-28集計) ----
    ...
    // 目的としたサテライトではない。過去の調査でも日本シリーズの候補店舗は見つかっていない
  ...
  },
  {
    ...
    bannerClass: 'ev-fst',
    // ---- FST 5.0 サテライト開催店舗(2026-08-27集計) ----
    ...
    //     const ids=new Set([...D.TOURNAMENTS,...D.RECURRING].filter(hit).map(t=>t.venueId));
    //     console.log([...ids].sort())"
    // 目視確認(TOURNAMENTS配列の7割ほど)では13店舗+note記載のみ2店舗(v28/v34)だったが、
  ...
  },
  {
    ...
    // 構造化データと一致(spadie-fukuoka-1st-content-brief.md参照)。
    //
...
//   これはバグではなく意図した仕様(2026-07-29)。将来「ズレている」と思って揃えないこと。
//
...
//   「翌日の朝6:00まで」に短縮した(仕様意図:「次の日の朝には消していい。これは大型大会も
...
// (2026-07-29に14日で設定 → 2026-09-01に30日へ変更)
...
// 掲載終了の「打ち切り時刻」。何時に消すかを表す定数(2026-09-06導入)。
...
//   「次の大会」を出し続けるため(2026-07-29導入)。バナーと一致しない日がある。
