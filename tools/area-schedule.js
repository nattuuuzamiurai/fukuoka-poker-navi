 ...
 *   1店しかないエリア(2026-08-29時点で博多・京築・筑豊)にエリアページを作ると、
 *   中身がその1店の店舗ページとほぼ同じになる。同じ内容のURLが2本ある状態は
 *   どちらの評価も上がらないうえ、読者にとっても回り道でしかない。
 *   店が2軒目が入った時点で自動的にページができる(手当ては不要)。
 *   ★2026-08-29: 「西中洲」もこの1店エリアだったが(梵 bon 西中洲のみ)、
 *   同じ住所(西中洲1-21)の CASINO BLOW 西中洲(v9)が最初から area:'中洲' で
 *   登録されていた(データ投入時からの不整合)。1店だけの「西中洲」に留めると
 *   同エリア他店カード・エリアページ・トップ/FSTページからのエリア経由リンクの
 *   どれも0本のまま孤立するため、既存の分類にそろえて v29 も area:'中洲' に統一した
 *   (Search Console実測で表示回数はあるのに掲載順位9〜12位のページの内部リンク強化・
 *   企画/開発タスク)。新事実の追加ではなく、既存データ(同一建物の住所)にもとづく
 *   分類の整合。AREA_SLUGS の 'nishi-nakasu' は将来また西中洲単独の店が増えたときのために
 *   残してあるが、2026-08-29時点で該当店は無い。
 *
 * 【sitemap に載せる条件は別 = 掲載中の日程が1件以上あること】
 *   店舗ページ(gen-sitemap.js の VENUE)と同じ考え方。日程0件のエリアページは
 *   実質「店の一覧」だけなので、AdSense審査に出す全URLの中に混ぜない(2026-07-30)。
...
/**
 * フッターの「エリアから探す」テキストリンク一覧(2026-08-28追加)。
