 ...
 * 共通ロジック。**`tools/import-waitinglist.js` の
 * 安全設計(upsert規則・安全弁)をそのまま踏襲**している。
 *
 * 【由来】
 *   もともとInstagram巡回を試作していた際に切り出したモジュール。Instagram巡回自体は
 *   採用しなかったが、この upsert ロジック自体は取得元に依存しない汎用部分なのでそのまま残す。
 *
 ...
 *
 * 【この店(venueId)以外・過去日には一切触れない】(import-waitinglist.js と同じ upsert規則)
 ...
 * 人が付けたタグ)は置き換え時に温存する。import-waitinglist.js(Waitinglist取込)と同じ語彙をそのまま使う
 ...
 * import-waitinglist.js の carryOver と同じロジック。
 *
