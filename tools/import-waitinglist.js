 ...
 *     【正常な他店の取込まで丸ごと止まる】(=運営の手入力を消している v3 の自動化が、
...
const STORES = [
  ...
  // 店名一致を確認済み(README「Waitinglist 自動取込」表)。2026-09-08有効化。
  ...
  //      【毎朝 exit 1 で永久に止まる】状態だった(v27 で再現。v22 では起きない)。今は修正済み。
  ...
  //
  //   [要確認・店名は一致するが「今日以降の件数」急減ガードに掛かるため保留(2026-09-08実測)]
  //   v33 / 4091897 … Poker room SKY。data.js の運用メモのとおり「月替わりのカレンダー画像で
  //     毎月告知」する店で、人が月初にひと月ぶんまとめて手入力する運用(2026-09-08時点で
  //     data.js に9/30まで18件の手入力あり)。Waitinglist API側は当月分を都度登録する運用で、
  //     同時点でAPIが返す「今日以降」はまだ6件(9/15まで)だった。重なる範囲(9/1〜9/11)は
  //     大会名・buyin・reentry/addonとも一致し内容の乖離は無い — が、この「18→6」という
  //     件数差だけで【急減ガード】(前回の50%未満)に抵触し、有効化した初回の朝から
  //     `--dry-run` 実測で即エラー(exit）になることを確認した。ガード自体は「その店を
  //     書き換えない」だけの安全な失敗だが、このままでは human が9月分を先読み入力して
  //     いる限り(=自動化したい当のもの)、日次botは9月末まで実質ずっとこの店をスキップし
  //     続ける。`--allow-shrink` で無理に通す・9月末の手入力が過去日に落ちてから有効化する
  //     等の対応が要るため、今回は有効化を見送り「要確認」として申し送る。
...
//   広げた瞬間に運営の入力が「機械のもの」に化けて静かに上書きされる
 ...
 *   「運営が DMM公式と突き合わせて直した手入力」を持つ店を有効化した瞬間に同じことが起きる。
...
async function main() {
  ...
  //      (品質チェックの変異試験で実証済み: 構造的な差し替えは検知できるが in-place は抜ける)。
...
//
// ★【require しても main() は走らない】(リスク台帳 #21・2026-08-05に解消)
//   以前はこのガードが無く、`node -e "require('./tools/import-waitinglist')"` のような
//   呼び方をすると【本物の data.js と本物のAPIに対して本番の取込みがそのまま走った】。
//   実際に一度事故になっている(構文チェックのつもりで require したワークツリーで
//   本番取込みが走った)。monitor-instagram-apify.js / import-venue-image.js と同じ形にした。
//
//   ★【このファイルは module.exports を持たない】★ 意図的にそのままにしてある。
//     exports を生やすと「require して関数だけ呼ぶ」経路が生まれ、毎朝06:23に本番稼働している
//     このツールの実行経路に手を入れることになる。ガードの目的は
//     【require を安全にすること】であって【require を使えるようにすること】ではない。
//     require した結果は空オブジェクトで、そこから呼べるものは何も無い。
...
//   ガードが外れていないことは tools/import-waitinglist.test.js の
//   「★require ガード」の3本が固定している(外すと落ちる)。
