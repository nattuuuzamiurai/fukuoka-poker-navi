...
/**
 * ============================================================
 * 【この下の3本は書き込みの「前」にある = 落ちても自分では直らない】
 * ============================================================
 * 呼び出し側はここが鳴ると `data.js` を1バイトも書かずに中止する。つまり
 * **次の実行も同じ入力で始まり、同じ理由で落ちる**（README リスク台帳 #17）。
 * これは意図した恒久停止装置である —— 鳴る条件は「マージの規則が破れた」だけで、
 * そのまま書き進めれば人の入力を壊したデータが公開されるため。
 *
 * 【良性の入力で鳴らないことの実測（2026-08-05）】
 *   実データに現れうる形だけを組み合わせた入力 12万件（1店が複数ブロック / 人の行 /
 *   控えのある機械の行 / 控えの無い wl- auto 行(seed) / 人が項目を直した行 /
 *   同じ (date,start) に人の行が2つ / start が空文字・キーごと欠落 / 日付欠落 /
 *   供給元から消えた行 / 抽出結果の id 重複 / 控えが空 / 取得に失敗した店が混ざる）
 *   に対して **発火0件**。詳細は README「書き込み前に `exit(1)` する検査の全数監査」。
 *
 * 【ここに4本目を足す担当へ】足す前に、上の監査の表に1行足せるかを確かめること。
 *   「良性の入力で鳴らない」を実測で示せないなら、それは検査ではなく恒久停止装置である。
 * ============================================================
 */
