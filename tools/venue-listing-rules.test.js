...
/**
 * venue-listing-rules.test.js — 店ごとの掲載ルール(運営判断)の両方向を固定する。
 *
...
test('★規則1: 参加費を記録しない店として v35 が登録されている(理由つき)', () => {
  ...
  assert.ok(r, 'v35 が対象から外れている(運営判断 2026-08-05)');
  assert.equal(r.venueId, 'v35');
  // 【理由は「運営判断」だけにしない】なぜそうするのかが無いと、後から誰も再評価できない。
...
test('★規則2/3(逆方向・これが本命): 語は【対象店以外】には当たらない', () => {
  ...
  // ★v18 は Instagram監視の対象5店(2026-09-01〜。以前はv34を含め6店)に入っている。全店に効かせると【自動経路が毎月
...
test('規則2/3: 部分一致であること(「大還元」を含む未知の名前も対象店では落とす)', () => {
  // 運営が挙げたのは `チップ大還元` と `スーパー大還元` の2通りだが、店が来月別の修飾を
...
test('規則2/3: 根拠の強さが規則ごとに記録されている(推定と不明を同じ顔で並べない)', () => {
  ...
  // 運営は「おそらくリングの話」と【推定】で言われている。確定した事実ではない。
  assert.match(orio.basis, /推定/, '推定であることが記録に残っていること');
  assert.match(orio.reason, /おそらく/, '運営の言葉づかい(推定)がそのまま残っていること');
  // 直方の華金は「なにかわからないので無視」= 内容不明が理由。
  assert.match(nogata.basis, /不明/, '「分からないから除外」という理由が残っていること');
  assert.match(nogata.reason, /なにかわからない/, '運営の言葉がそのまま残っていること');
});
