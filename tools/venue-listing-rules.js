 ...
 * 2026-08-05、試験実行(run `30973996821`・dry-run・書き込み0)の結果42件を見た運営から
 * 出た3件の指示がその第1号である。
 *
 * ============================================================
 ...
 * ============================================================
 * 運営の言葉は3件それぞれ強さが違う。**同じ「除外」でも、確定した事実に基づくものと
 ...
 * 表明することになる。**運営にも「無料の0円も含めて全部消す」で確認済み**。
 *
 ...
 * 大会名はそのまま載せる。参加費(円)の主張ではないので触っていない。
 *
 * 【この規則が及ばない範囲】
 *   ・`addon` / `stack` / `guarantee` は対象外(対象は参加費のみ)。
 ...
 *   `大還元` は `チップ大還元` `スーパー大還元` の両方に当たる(運営に確認済み)。
...
const NAME_EXCLUSIONS = [
  {
    ...
    instructedAt: '2026-08-05',
    reason:
      '運営の言葉は「折尾のチップ大還元は【おそらく】リングの話なのでこれは無視で」。' +
  ...
  },
  {
    ...
    basis: '内容が不明(運営が把握できていない)',
    instructedAt: '2026-08-05',
    reason:
      '運営の言葉は「直方の華金は【なにかわからない】ので無視で」。' +
