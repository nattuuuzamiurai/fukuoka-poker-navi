 ...
 * 配列に正規化する。
 *
 * 【由来】以前試作していた `tools/instagram-vision.js` の抽出ロジックをそのまま引き継いだもの。
 * 呼び出し元が「店舗が直接送ってきた画像ファイル/投稿リンク」(`tools/import-venue-image.js`)
 * になっただけで、画像→JSON の変換ロジック自体は同じ。フィード一覧から新着を判定する
 ...
 * 余裕がほぼ消える。これは以前差し戻された構図そのものが規模を縮めて残ったもの。
...
async function extractTournaments(imageBuffer, { postedDateHint, mediaType } = {}) {
  ...
  const user = [
    ...
    'start は必ず画像の時刻表記から読み取ってください。読み取れない場合だけ null にしてください' +
      '(推測で 00:00 などを入れないこと)。',
    '【大会名は画像の表記をそのまま写してください】綴りを直したり、省略を補ったりしないでください' +
      '(店ごとに表記が違うのが正常です)。読み取れない文字があっても推測で埋めないこと。',
