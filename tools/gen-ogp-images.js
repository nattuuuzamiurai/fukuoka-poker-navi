 ...
 * (2026-08-28追加。調査の結果、トップ・エリア・店舗ページにog:imageが一切無く、
 * 既存のイベント4ページのバナー画像も1024×412でアスペクト比が推奨値とズレていた)。
 * デザイン仕様は ogp-design-spec.md(コンテンツ制作作成)を参照。
 *
 * 【依存ライブラリについて】
 ...
 *   個別に再配置することはできない)。前回はこの1枚を縮小配置し外周に余白を足しただけの
 *   対応にとどめていたが、それでは「元アートワーク内で既に切れている」問題は解決しない
 *   (レビュー指摘・2026-08-28)。そのため buildWjpt() は他イベントと異なり元バナーを一切使わず、
...
// 下段: 大会名・日程・動的な補足情報(動的な値は手打ちしない方針)。
...
// ---- 4. 日本シリーズページ用OGP ----
// 【結果・優勝者情報は入れない】運営方針・2026-08-28。WJPT・日本シリーズの結果調査・記載は
...
// 【2026-08-28 レビュー指摘対応】元バナー画像は使わず、他イベントページと同じ配色
