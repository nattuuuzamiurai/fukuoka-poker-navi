 ...
 *   webcoin-regulation-content-draft.md（品質チェック2ラウンド・レビューGO済み、2026-09-14）。
 ...
 *   このページは逆に「特定の店舗を評価しない」という立ち位置の記事で、レビューの申し送りにより
 ...
 *   というのがレビューの判断。冒頭（H1直後のlead段落の直後、.tbaの直前）にも同じ .disclaimer
 *   を追加し、末尾の .disclaimer はそのまま残す（二重掲載で問題ない、とレビュー確認済み）。
 *
 * 使い方:
...
// ---- 本文の組み立て ----
// 【2026-09-26改訂・本題への導線を圧縮】この記事の結論(最も価値のある要約)を、目次より前に
// 移動した。以前は免責文言・最終確認日のお知らせの直後に10項目の目次を挟んでから結論を
// 示していたため、「まずここだけ読みたい方」にとって前置きが長くなっていた。目次自体は
// 章ジャンプの手段として引き続き結論の直後に残す(内容・アンカーIDとも変更なし)。
// ★このコメントは function 定義の外側(JS の // コメント)に置くこと。以前 <!-- --> の
//   HTMLコメントとして buildBody() の返り値(テンプレートリテラル)の中に書いていたため、
//   生成後の公開ページ(guide/webcoin-regulation/index.html)のHTMLソースにそのまま出力され、
//   View Sourceで内部の改訂理由が読める状態になっていた(README「コード・データファイルへの
//   内部情報の記載について」に反する)。同じ趣旨のコメントを残したい場合は、必ずテンプレート
//   リテラルの外側に書く。
function buildBody() {
  return `
...
// ---- 検査(レビュー申し送り1: 店舗一覧・個別店舗ページへのリンクを一切張らない) ----
function verify(html) {
  if (/href="[^"]*\/venues\/[^"]*"/.test(html)) {
    throw new Error('gen-guide-webcoin-regulation.js: この記事から店舗ページ(/venues/)へのリンクが検出されました。'
      + 'レビューの申し送り(特定店舗を評価しない立場を崩さないため、この記事からのリンクは禁止)により、'
      + '本文からこのリンクを取り除いてください。');
  }
  ...
  if (!html.includes('本記事は、特定のアミューズメントポーカー店・サービスの合法性を当サイトが判定・保証するものではありません')) {
    throw new Error('gen-guide-webcoin-regulation.js: レビュー申し送り2の免責文言が本文に見つかりません。');
  }
  // レビュー NO-GO(2026-09-14)差し戻し対応: 免責文言はH1直後(冒頭)にも無ければならない。
  ...
  if (h1 < 0 || firstH2 < 0 || !html.slice(h1, firstH2).includes('class="disclaimer"')) {
    throw new Error('gen-guide-webcoin-regulation.js: 冒頭(H1直後・最初の見出しより前)に .disclaimer が'
      + '見つかりません。レビューの申し送り(2026-09-14 NO-GO差し戻し)により、免責文言は記事末尾だけでなく'
      + '冒頭にも置く必要があります。');
  }
}
