...
const { venueJsonLd, validateUnverifiedFlags, validateGeoFlags, validateHoursSpec, validatePriceRangeSpec } = require('./venue-jsonld.js');
validateUnverifiedFlags(VENUES);
validateGeoFlags(VENUES);
validateHoursSpec(VENUES);
validatePriceRangeSpec(VENUES);
...
const VENUE_CSS = `  .vp-sub{font-size:.9em;color:var(--mut);margin-bottom:14px}
  ...
  /* ホットペッパー グルメの店舗写真(2026-09-12・PR #91の情報カードの上に配置)。
     ...
     発生しない)。
     【2026-09-16・aspect-ratio を追加してCLS対策(マーケティング部SEO調査・PR #103/#108のフォロー
     アップ)】従来は width/height属性もaspect-ratioも無く、height:auto相当だったため、画像の
     読み込みが完了するまで高さが確定せずレイアウトシフト(CLS)が起きうる状態だった。トップページの
     店舗カード(.vc-thumb・aspect-ratio:16/9)・トーナメント一覧アイコン(.tc-thumb)は既に固定比率で
     対策済みなのに、ここだけ対策漏れだった指摘への対応。.vc-thumbと同じ 16/9 にそろえる。
     ★ max-height:320px は残す。aspect-ratio と max-height を両方指定した場合、CSSの計算順は
       「width(100%)→aspect-ratioからheightを算出→max-heightで上限クランプ」であり、どちらの値が
       最終的に効いても【画像の実サイズを見ずに算出できる】ため、幅の狭い画面(aspect-ratioが効く)・
       広い画面(max-heightが効く)のどちらでもCLSは起きない(高さが常にCSSだけで決まる)。
     ★ 正方形に近いロゴ画像は 16:9 の横長枠の中で contain 表示され、左右に余白ができる
       (= ピラーボックス)。以前(aspect-ratio無し)は画像の実サイズがそのまま枠になっていたため
       正方形は正方形のまま出ていたが、この変更で見え方は変わる。余白は他カードと同じ
       var(--sur)(白地)で埋まるため浮いた印象にはならない、と確認済み(ローカル確認)。
       横長の実写真(ホットペッパー由来)は元々 16:9 に近い比率が多く、この変更後もほぼ違和感なく
       表示される。 */
  .vp-photo{margin:0 0 14px}
  .vp-photo img{display:block;width:100%;aspect-ratio:16/9;max-height:320px;object-fit:contain;background:var(--sur);border:1px solid var(--bor);border-radius:var(--r);box-shadow:var(--sha)}
...
function buildVenue(v) {
  ...
  const descLead = badge && badge.kind === 'fst' ? 'FST 5.0のサテライト（チケット獲得トーナメント）を開催中の店舗です。'
    ...
    : '';
  // altNames(店舗が自身のSNS等で使っている別表記)を .vp-sub 末尾に控えめに1回だけ添える
  // (2026-09-13新設・裸の店名検索対策)。JSON-LDのalternateName(tools/venue-jsonld.js)と
  // 同じ data.js の値をそのまま使う(表記の裏取りはしない・data.js を書く人が確認済みの値のみ)。
  ...
  if (v.preopen) {
    ...
    sub = `${esc(v.area)}のポーカースポット${v.access ? `（${esc(v.access)}）` : ''} — オープン予定${altNamesNote}`;
  } else if (rows.length) {
    // ★ 2026-09-13改訂(裸の店名検索対策)。以前は「◯◯のポーカートーナメント日程」のように
    //   日程情報を主役にした構成で、「その店は何?」という裸の店名検索の検索意図に対して
...
<iframe class="vp-map" src="${esc(mapEmbedUrl(v))}" title="${esc(v.name)}の地図" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>` : '';
  ...
  //   - 掲載0件の店に「※ この一覧は◯月〜◯月の掲載分です」と出す(以前の全店共通期間の実装)
  //     … 1行も載っていないのに期間を掲載したことになり、事実と合わない
  //   - 定期開催しかない店に「※ この一覧は2026年7月の掲載分です」と出す
  //     … この期間は「毎週の定期開催を1ヶ月ぶん展開して見せるための代表期間」であって、
  //       その月の開催予定という意味ではない。しかも起点は全店の最古月なので放っておくと動かず、
  //       2027年に読んでも「2026年7月の掲載分です」と出続ける(venue-schedule.js の代償の項を参照)。
  //       そこで【日付を断定しない文言】にし、月は「展開した例」であることが分かる形でだけ添える。
  const NOTE_JS_TAIL = 'JavaScriptが有効な環境では、読み込み時に最新の掲載データから<b>今日以降</b>の日程に差し替わります。';
  let schedTitle, schedNote;
  if (!RANGE) {
    schedTitle = 'トーナメント日程';
    schedNote = `※ ${NOTE_JS_TAIL}`;
  } else if (RANGE.recurringOnly) {
    schedTitle = 'トーナメント日程（毎週の定期開催）';
    schedNote = `※ この店舗は曜日固定の定期開催のみを掲載しています。下の表はそれを1ヶ月ぶん（${esc(RANGE.label)}）`
      + `日付に展開した例で、特定の月の開催予定を示すものではありません。${NOTE_JS_TAIL}`;
  } else {
    schedTitle = `トーナメント日程（${esc(RANGE.label)}の掲載分）`;
...
//   = 検査がデータとディスクの不一致を見逃す。
//   BAR BETTY(v32)が2026年9月の閉店に伴い2026-09-01に VENUES から削除された際、
//   実際にこの検出が動いて venues/bar-betty-nakasu/ を削除した(社長情報・2026-07-29の実行)。
//
// 【events/ 側は対象外】
