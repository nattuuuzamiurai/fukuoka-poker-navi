...
function ledgerRows() {
  ...
  // (例: 定型「検知器を試したら配線にも変異を当てる」の一覧)。全体を走査すると
  // 台帳の行数が水増しされ、内訳表を正しく直しても落ち続ける。実際に別の変更を
  // マージした時点で 25行 → 28行 と誤認して落ちた(2026-08-05)。台帳の「### 一覧」節に限定する。
 ...
 * を書いた。1度目の訂正で1つ取りこぼし、品質チェックの指摘で2度目を入れている。
 *
 * 【★assert は根治ではない（2026-08-05・レビュー）★】
...
test('リスク台帳: 地の文に手書きの件数を書かない（表を正とする）', () => {
  ...
  // 【★「1行も見ない」だけでなく「半分しか見ない」も止める（2026-08-05・品質チェック）★】
  //   上の `n >= 5` は範囲がゼロになる壊れ方しか捕まえない。例えば `### 一覧` を `##` に
  //   格上げすると走査は静かに縮むが、5行は残るのでテストは緑のまま通ってしまう。
  //   台帳の【最後の節】の見出しに到達していることを、名指しで確かめる。
  assert.ok(
    prose.some((p) => p.region === 'ledger' && p.text.includes('### `実測` の語法')),
    '台帳の末尾まで走査できていません(台帳の最後の節「### `実測` の語法」に到達していない)。' +
    '見出し構成を変えたらこのテストも直すこと'
  );
  const violations = [];
