...
// 回帰テスト(2026-07-31): 公開前チェックで発見した致命的バグの再発防止。
...
// 重複ID(2026-07-31 / 品質チェックの指摘)。Visionが同じ行を2回返す、あるいは start が
...
test('runMonitor: 全行が人の日時訂正と衝突した投稿は、異常ではなく別のバケツに数える', async () => {
...
function makeTempRepoRoot(extraTournaments = []) {
  ...
  for (const f of [
    ...
    // 店ごとの掲載ルール。monitor-instagram-apify.js が require するので必須。
 ...
 *
 * 【★Proxy は spread / `Object.keys` / `in` を観測できない(2026-08-04・品質チェックの申し送り)】
...
// 実際、品質チェックの走査で「Vision 0件 のログ行に本文を出す」変異だけが生き残った。
...
test('★品質: 名前しか読めない行も捨てずに取り込む(⚠は付けない)', () => {
  ...
  // 実測で44行中42行(95%)に点灯し、印としての意味を失っていた。運営の基準では
...
// 「時刻も参加費も無い行」は運営の基準では平常なので付けない(旧判定は44行中42行=95%に点灯し、
...
test('★品質: 名前だけの行・時刻だけの行には⚠を付けない(常時点灯する警報を作らない)', () => {
  // 前回の試験実行では 44行中42行(95%)に⚠が点灯していた。運営の基準では
...
test('★警告: 平常から大きく外れたときだけ ::warning:: を出す', () => {
  ...
  // 【★到達不能な分岐を「ある」と書かない】(2026-08-04・レビューの指摘)
...
test('★探索(#13の再現): 本物のカレンダーが打ち切りの後ろに隠れていることを数える', () => {
  // レビューの再現そのもの: 8月カレンダー(20日付・08-01投稿)の【後に】
  ...
  // 定義上存在しない。つまりレビューが指定した数字【だけ】を見ると 0 = 安全に見えるが、
  ...
  assert.equal(m.aheadCalendars, 0, 'レビュー指定の数字。この形では構造的に0になる');
...
//
// 【同じ形を3回踏んでいる】保護を外すと破壊ではなく重複になり、破壊しか見ない
// 検査が素通りしたケース、本番ツール側の報告の固定が抜けており出力を消しても緑だったケース、そして今回。
...
// ============================================================
// 【なぜ ahead/behind の件数では足りないか(2026-08-05・レビューの指示)】
...
test('★形状比較: 採用より強い投稿が後ろに居たら印を付ける(#13 の署名)', () => {
  // レビューの再現そのもの: 偽陽性(異なる日付5)が本物(20)を追い越している
  const m = monitor.probeMetrics({
    probeVerdicts: [
      { index: 0, permalink: 'p/SERIES/', kind: 'calendar-current', dominantMonth: '2026-08', distinctDates: 5, spanDays: 28 },
      { index: 1, permalink: 'p/REAL/', kind: 'calendar-current', dominantMonth: '2026-08', distinctDates: 20, spanDays: 30 },
...
test('★形状比較: 後ろの本物が【偽陰性】でも効く(分類を経由しないこと)', () => {
  ...
  // 支配月は past なのでレビューが提案した条件(1)は満たさない。それでも
...
test('★形状比較: 同点は「最強」に倒す / 形が確定していない投稿は母集団に入れない', () => {
  ...
  // 【★同点の相手が「採用より新しい位置」に居る場合★】(品質チェックが見つけた素通りの穴)
...
/** レビューが構成した盲点の再現(実測ではない): 偽陽性5日付 / 後ろの本物20日付を past と誤読。 */
...
test('★形状比較(盲点): 支配月を誤読された本物を追い越している形には★が付く(実測4件と両立)', () => {
  ...
  assert.deepEqual(m.signatureReasons, ['half'], 'レビューの条件(1)は満たさず、半分以下の条件だけで立つこと');
  assert.match(
...
test('★形状比較: 当月以降を名乗る投稿が採用より大きければ、半分以下でなくても★(レビューの条件)', () => {
  const m = monitor.probeMetrics({
    probeVerdicts: [
  ...
  const run3 = await monitor.runMonitor(
    {
      ...
      // 「人の行を守って見送り」に入る(machine-write-state.js の規則)。ここで見たいのは前者。
...
// ============================================================
// 店ごとの掲載ルール(2026-08-05追加) — 取込み経路での【配線】
// ============================================================
...
test('★CLI: 掲載ルールの節は【毎回】実行ログに出る(0件の回でも)', () => {
  const r = runCliWithMutation(() => {});
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.stdout, /=== 店ごとの掲載ルール\(tools\/venue-listing-rules\.js\)/);
...
test('★CLI(実データの形): v40の「大還元」が落ち、v35の参加費が消えて、他店は無傷', () => {
  // 試験実行 run `30973996821` で運営が指摘した3件を、1回のCLI実行で同時に再現する。
  const root = makeTempRepoRoot();
  fs.writeFileSync(
    path.join(root, 'tools', 'fetch-venue-posts-apify.js'),
...
// ============================================================
// 【当月取得済みラッチ】2026-08-05追加(Vision呼び出しの削減)
// ============================================================
// 【このブロックが守っているもの】
