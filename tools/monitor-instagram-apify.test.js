...
// 回帰テスト(2026-07-31): 品質チェックがPR #16で発見した致命的バグの再発防止。
...
function makeTempRepoRoot(extraTournaments = []) {
  ...
  for (const f of [
    ...
    // 店ごとの掲載ルール(運営判断)。monitor-instagram-apify.js が require するので必須。
...
// ============================================================
// 店ごとの掲載ルール(運営判断・2026-08-05) — 取込み経路での【配線】
// ============================================================
...
test('★CLI: 掲載ルールの節は【毎回】実行ログに出る(0件の回でも)', () => {
  const r = runCliWithMutation(() => {});
  assert.equal(r.status, 0, r.stderr);
  assert.match(r.stdout, /=== 店ごとの掲載ルール\(運営判断/);
...
// ============================================================
// 【当月取得済みラッチ】運営判断・2026-08-05(Vision呼び出しの削減)
// ============================================================
// 【このブロックが守っているもの】
