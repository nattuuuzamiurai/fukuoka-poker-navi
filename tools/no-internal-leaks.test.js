...
const LEAK_PATTERNS = [
  ...
  // 内部レビュー・指摘のやり取りをそのまま引用した経緯語り(2026-09-19、site-shell.js /
...
test('検知パターンの回帰確認: NGワードを含む文言は検知され、実ファイルの現在の文言は検知されない', () => {
  const beforeFix = [
    '(テスト用ダミー文言)セッションCookie注入を伴う設計は',
    '検知回避のリスクがあるため採用しなかった。',
  ].join('\n');
...
// ============================================================
// 回帰確認: site-shell.js / index.html に残っていた内部レビューの指摘文面の
// 原文引用(2026-09-19に是正)が、検知パターンで再現できることを固定しておく
// ============================================================
test('検知パターンの回帰確認: 内部指摘の原文引用は検知され、圧縮後の文言は検知されない', () => {
  const beforeFix = [
    '【2026-09-14改修】「案内があまり目立たない、デザインに甲乙つけてわかりやすく」という指摘を受けて改修。',
    '★2026-09-14「この説明文いらない」という指摘を受け理由確認の結果「(SEO用途なら)見た目は',
    '最初は見えないよう折りたたんでほしい」とのことだったため、常時表示の<p>から変更した。',
  ].join('\n');
  const afterFixSamples = [
    '【2026-09-14改修】視認性向上のためデザインを見直した。',
    '★2026-09-14、SEO上の役割は維持しつつ見た目には常時表示しない形にするため、常時表示の<p>から変更した。',
  ].join('\n');
