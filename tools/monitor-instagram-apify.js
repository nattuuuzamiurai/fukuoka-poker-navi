 ...
 * 漏れた定休日にも詳細未定の大会にも⚠が付いていた。運営の基準
...
function toTournament(t, venueId) {
  ...
  // 「時刻も参加費も無い行」は運営の基準では【平常】なので印を付けない(判定の根拠は
 ...
 * (レビューの再現: シリーズ告知5件=異なる日付5・広がり28日 が本物の月間カレンダーを追い越した)
...
function probeMetrics(summary) {
  ...
  return {
    ...
    // ★レビューが指定した数字。位置=採用より新しい / 形=カレンダー判定を満たす
 ...
 *
 * 【なぜ分類ではなく「形」を比べるのか(2026-08-05・レビューの指示)】
 ...
 *   つまり ★ は放っておいても改善せず、
 *   **誤警報が既知の警報を、人は開かなくなる**(2026-08-05・レビュー)。
 *
 ...
 *   (1) **窓内の最大の支配月が、採用の支配月以上**(レビューの提案)
 *       — 当月以降を名乗る投稿が採用より大きい = 分類が正しい前提での #13 そのもの。
 *   (2) **採用の異なる日付が、窓内の最大の【半分以下】**
 *       — (1) だけだと、レビュー自身が指摘した盲点(**本物が支配月を誤読されて past 扱い**)に
 ...
 *   ・**鳴る側はレビューが構成した再現シナリオ**(実測ではない): 偽陽性のシリーズ告知は
 ...
 *
 * 【★採用側の permalink も必ずこの行に載せること(2026-08-05・品質チェックの指摘)★】
 ...
 *
 * 【★この警告で捕まえられるのは【下振れ】だけ(2026-08-04・レビューの指摘で訂正)】
