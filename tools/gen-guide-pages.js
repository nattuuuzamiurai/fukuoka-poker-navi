 ...
 *   ★2026-09-12改訂: 運営フィードバック「内容が薄すぎる」を受け、旧「初心者が選ぶときに
 ...
 *   出典の性質が異なる。公開前に品質チェックでの裏取りが必要（原稿冒頭の開発向けメモを参照）。
...
const HIGHLIGHT_STORES = [
  { id: 'v6', intro: '公式Threadsのプロフィールによると、初心者講習を行っているとのことです。' },
...
const GUIDE_CSS = `  .gd-card{background:var(--sur);border:1px solid var(--bor);border-radius:var(--r);box-shadow:var(--sha);padding:14px 15px;margin-bottom:10px}
  ...
  /* 【2026-09-13】categoryChips()/NOT_FOUND_IDS が出す <ul class="vp-list"> の
     チップ/タグ表示は tools/gen-venue-pages.js の VENUE_CSS に定義があるが、こちらの
     GUIDE_CSS への移植を忘れていた(無地の黒丸箇条書き+下線リンクとして表示されていた)。
...
function categoryFeaturedCards(featured) {
  // 【空ガード・2026-09-13】featured が空のカテゴリー(全店chips化した「リングゲーム、初心者」
  // 「バカラ・ブラックジャック」)で空の <div class="vp-cards"></div> だけを出力しないようにする。
  // 空タグでも見た目には出ないが、無意味なマークアップを埋め込まないため素通りさせる。
...
// 【なぜ足すか】マーケティングの分析(Search Console実測)により、「福岡 ポーカー」
...
// 参照する(店名を文章中に列挙すると「文字だらけ」に戻るため、運営指摘〔2026-09-12〕を踏まえて分離)。
...
const FAQ_ITEMS = [
  ...
  // 【2026-09-13新設・マーケティング分析】Search Console実測で「福岡 ポーカー リング」の
...
// ---- ページ全体 ----
// 【2026-09-13・マーケティング分析にもとづくタイトル/description差し替え】
...
function buildGuidePage() {
  ...
  // 【2026-09-26改訂・本題への導線を圧縮】規制動向の告知(ウェブコイン解説記事への導線)は、
  // 以前は免責文言の直後(=目的別カテゴリーより前)に置いていたため、このページの本題である
  // 「福岡のポーカー店を目的別に探す」に読者が辿り着くまでの前置きが長くなっていた。
  // 内容は変えず、全一覧の後・FAQの前(補足情報として読む位置)に移動する。
  const regulationNotice = `<div class="tba"><b>2025年12月、警視庁がアミューズメントカジノ店舗の一斉調査を実施するなど、業界を取り巻く規制の動きが続いています。</b>「ウェブコイン」を中心とした規制の背景・プレイヤーとして確認しておきたいポイントは<a href="/guide/webcoin-regulation/">こちらの解説記事</a>でまとめています。</div>`;
