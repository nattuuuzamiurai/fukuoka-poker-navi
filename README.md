...
- **常時点灯する警報を作らない(2026-08-01追加)**
  ...
  - **値そのものではなく値の【変化】を見る**。割合は必ずログに残し、
    ...
    下がったなら店が時刻を書き始めた(良い変化)、上がったならVisionが読めなくなった、
    どちらも**画像を1枚見れば分かる**。平常値が変われば定数を更新する(警告文にも書いてある)
- **大会名の綴り誤りは直さない(2026-08-01の判断)**
...
- **抽出品質の検査(2026-08-01追加)**: **機械的な検査を全部通っても、中身が公開に耐えるとは限らない**
  ...
  - **★定休日のマス — 「構造で落とす」は失敗した。多層防御にした**
    ...
    - **★ただし「名前由来かもしれない参加費」は証拠に数えない**。Visionは画像に金額が無くても
  ...
  - **開始時刻が読めない行の割合が高ければ `::warning::`**。「店が時刻を書いていない」のではなく
    **「Visionが時刻の列を読めていない」**可能性が高く、それは投稿画像を見ないと分からないため
