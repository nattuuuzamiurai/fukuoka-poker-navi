...
- **⚠ 静的ページの再生成は、このワークフローには入っていない（別PRに切り出し）**: 自動取込で `data.js` が変わると、そのスナップショットである `venues/<slug>/index.html` と `sitemap.xml` は古いままになる。**当面は従来どおり人が `node tools/gen-venue-pages.js .` を実行すること**（上記「`data.js` を更新したら」）。
  日次の自動再生成をこのワークフローに入れなかった理由は、現在の `tools/venue-schedule.js` の `dataRange()` が**全店グローバルの日付範囲**を返すため:
  - 1店のトーナメントが1件増減しただけで**35ページ全部**が書き換わる（実測: 9月分の1件が消えるだけで36ファイル、2027年3月の大会が1件載ると `venues/` 全体が 1.1MB → 1.4MB）
  - 9月分があるのは m HOLD'EM 中洲だけなのに、他店のページにも「7月〜9月の掲載分です」と出て9月の定期開催行が焼き込まれる（見出しが不正確になる）
  - 無人の日次pushで毎朝35ファイルを書き換えると、`venues/` を触る他の作業と競合が常態化する

  `dataRange()` を**店舗別**にすれば見出しの不正確さと波及の両方が同時に解消し、v3の変更は `m-holdem-nakasu` にしか届かなくなる。その修正とセットで日次再生成を有効化する別PRを立てる。
