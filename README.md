...
- **抽出品質の検査(2026-08-01追加)**: **機械的な検査を全部通っても、中身が公開に耐えるとは限らない**
  ...
  - **★定休日のマスは「語」ではなく「構造」で落とす**。休業を表す言い方は
    「休み」「お休み」「定休日」「CLOSED」「×」…と**無限にあり語リストでは必ず漏れる**。
    代わりに**トーナメントである積極的な証拠**(開始時刻 / 参加費 / スタック / 保証額)を要求し、
    **1つも無い行は大会として扱わない**。画像の見出し(`月間TOURNAMENT`)も同じく落ちる。
    **語彙に依存しないので表記が変わっても効き続ける**
